import React, { useState, useEffect } from 'react';
import { useNavigate } from 'react-router-dom';
import Card from '../../components/common/Card';
import Table from '../../components/common/Table';
import Button from '../../components/common/Button';
import Input from '../../components/forms/Input';
import Badge from '../../components/common/Badge';
import accountingService from '../../services/accounting';
import './InvoicesList.css';

const TaxGroupList = () => {
    const navigate = useNavigate();
    const [taxGroups, setTaxGroups] = useState([]);
    const [loading, setLoading] = useState(true);
    const [searchTerm, setSearchTerm] = useState('');
    const [filterStatus, setFilterStatus] = useState('all');

    useEffect(() => {
        fetchTaxGroups();
    }, [filterStatus]);

    const fetchTaxGroups = async () => {
        try {
            setLoading(true);
            const params = {};
            if (filterStatus === 'active') params.is_active = true;
            if (filterStatus === 'inactive') params.is_active = false;
            const data = await accountingService.getTaxGroups(params);
            setTaxGroups(data.results || data);
        } catch (error) {
            console.error('Error fetching tax groups:', error);
        } finally {
            setLoading(false);
        }
    };

    const filteredTaxGroups = taxGroups.filter(tg =>
        tg.group_name?.toLowerCase().includes(searchTerm.toLowerCase()) ||
        tg.description?.toLowerCase().includes(searchTerm.toLowerCase())
    );

    const getStatusBadge = (isActive) => {
        return isActive 
            ? { variant: 'success', label: 'Active' }
            : { variant: 'secondary', label: 'Inactive' };
    };

    const columns = [
        { key: 'group_name', label: 'Group Name', sortable: true },
        { key: 'description', label: 'Description', sortable: true },
        { 
            key: 'total_tax_rate', 
            label: 'Total Rate', 
            sortable: true, 
            width: '120px',
            render: (value) => `${value ? value.toFixed(2) : '0.00'}%`
        },
        { 
            key: 'items', 
            label: 'Tax Items', 
            width: '100px',
            render: (value) => value ? value.length : 0
        },
        { 
            key: 'is_active', 
            label: 'Status', 
            width: '100px',
            render: (value) => { 
                const badge = getStatusBadge(value); 
                return <Badge variant={badge.variant} size="sm">{badge.label}</Badge>; 
            } 
        },
    ];

    return (
        <div className="journal-entry-list-page">
            <Card 
                title="Tax Groups (V2)" 
                subtitle="Manage compound tax groups"
                action={
                    <Button variant="primary" size="sm" onClick={() => navigate('/dashboard/accounting/tax-groups/new')}>
                        + New Tax Group
                    </Button>
                }
            >
                <div className="invoices-filters">
                    <div className="invoices-search">
                        <Input 
                            placeholder="Search tax groups..." 
                            value={searchTerm} 
                            onChange={(e) => setSearchTerm(e.target.value)} 
                        />
                    </div>
                    <div className="invoices-filter-buttons">
                        <Button 
                            variant={filterStatus === 'all' ? 'primary' : 'ghost'} 
                            size="sm" 
                            onClick={() => setFilterStatus('all')}
                        >
                            All
                        </Button>
                        <Button 
                            variant={filterStatus === 'active' ? 'primary' : 'ghost'} 
                            size="sm" 
                            onClick={() => setFilterStatus('active')}
                        >
                            Active
                        </Button>
                        <Button 
                            variant={filterStatus === 'inactive' ? 'primary' : 'ghost'} 
                            size="sm" 
                            onClick={() => setFilterStatus('inactive')}
                        >
                            Inactive
                        </Button>
                    </div>
                </div>
                <Table 
                    columns={columns} 
                    data={filteredTaxGroups} 
                    loading={loading} 
                    emptyMessage="No tax groups found" 
                    onRowClick={(tg) => navigate(`/dashboard/accounting/tax-groups/${tg.id}`)}
                />
            </Card>
        </div>
    );
};

export default TaxGroupList;
