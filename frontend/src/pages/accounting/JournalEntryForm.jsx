...
const JournalEntryForm = () => {
        ...
        try {
            ...
            if (data.entries && data.entries.length > 0) {
                setEntries(data.entries.map(e => ({
                    ...
                    description: e.description || '' // Assuming description exists on entry? Model doesn't have it on entry, only narration on header.
                    // Wait, VoucherEntryV2 doesn't have description!
                    // I should check the model again.
                    // VoucherEntryV2 has cost_center and department, but no description.
                    // The form has description field.
                    // I should remove description from form or add it to model?
                    // Let's check model.
                })));
            }
        } catch (error) {
    ...
    const handlePost = async () => {
        ...
        try {
            ...
            fetchVoucher(); // Refresh to show posted status
        } catch (error) {
    ...
    const generateVoucherNumber = () => {
        ...
        const num = `JE-${timestamp.toString().slice(-8)}`;
    ...
    return (
        <div className="invoice-form-page">
            <Card title="New Journal Entry" subtitle="Create a double-entry voucher">
